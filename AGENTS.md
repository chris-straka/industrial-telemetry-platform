...
6. Postgres has a unique `MessageId` index, so a Kafka replay is a no-op.
7. An anomaly row and its alert outbox row commit in one Postgres transaction. The publisher may
...
A cloud rejection is explicit permanent loss, not success: it is logged and metered so a poison
row cannot block every newer reading. Gateway admission and ingestion validation should remain
...
Docker Compose is the supported runnable path. The Helm/Tilt/Terraform material is an unfinished
prototype and is not a production deployment claim. Do not say it is deployable until chart
rendering, secrets, persistent edge identity/storage, Terraform state/backend, and CI plans are all
validated. Authentication and mTLS are also still open work; local ports are loopback-bound but
service-to-service traffic is plaintext inside the Compose network.

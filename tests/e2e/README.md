...
1. the edge queues readings while ingestion is stopped;
2. the SQLite queue survives an edge restart and drains after ingestion recovers;
3. Postgres contains exactly one row per submitted `MessageId`;
4. Diagnostics rewinds and later persists a reading consumed during a Postgres outage;
5. malformed Kafka JSON is durably copied to `telemetry-events-dlq` before its source offset is
   committed; and
6. a pending alert outbox row survives a Kafka outage and publishes after broker recovery.
